...
public class MasterServlet extends HttpServlet {
  ...
  public void doPost(HttpServletRequest request, HttpServletResponse response) 
	  ...
	  if(servletPath.equalsIgnoreCase(RUNWORK))
		  try {
			  ...
			  iter_num = Integer.parseInt(request.getParameter("iternum"));
			  ...
			  jobStatus = new JobStatus(inputDir, mapNum, redNum, map, iter_num);
  ...
  private void showStatus(HttpServletRequest request,
	...
	try {
		...
		out.println("Iteration Number:<input style=\"width: 500px;\" type=\"text\" name=\"iternum\"></br></br>");
  ...
  private void processWorkerStatus(HttpServletRequest request) {
	  try {
		  ...
		  LogClass.info("Worker: " + ip + "; Status: " + status);
			  ...
			  if(map != null && map.containsKey(ip)) {
				  ...
				  if(flag == false)  {
					  ...
					  if(leftNum == 0) {
						  ...
						  LogClass.info("Master Finish one round");
					  }
				  }
			  }
		  }
