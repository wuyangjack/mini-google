...
public class CrawlerThread implements Runnable{
  ...
  public void run() {
      while(! crawler.isDone()) {
          try {
              ...
              System.out.println("Thread " + Thread.currentThread().getId() + " Thread Error");
              e.printStackTrace();
          }
      }
  }
  ...
  public void init(){
      try {
          ...
          while(! crawler.isDone()) {
              ...
              System.out.println("Thread " + Thread.currentThread().getId() +" Crawled Success Number: " + crawler.getCount() + "; Dequeu: " + number);
              ...
              if(delay > 0) {
                System.out.println("Thread " + Thread.currentThread().getId() + " Waiting for Crawl-Delay: " + delay + " milliseconds");
                Thread.sleep(delay);
              }
              else if(delay == 0)
                Thread.sleep(500);
              ...
              int depth = crawl_url.getDepth();
              System.out.println("Thread " + Thread.currentThread().getId() + " Depth: " + crawl_url.getDepth());
              // if we have visited the url
              UrlText ut = database.getUrl(encoded_url);
              if(ut != null && ut.isVisited() == true)
            	 continue;
              ...
              System.out.println("Thread " + Thread.currentThread().getId() + " HeadNode status: " + head_node.getState() + "; Type valid: " + head_node.typeValid());
              Document doc = downloadUrl(head_node);
              if(doc == null)
                continue;
              else {
                crawl_url = new CrawlUrl(head_node.getAbsoluteUrl(), depth);
  ...
  public CrawlInfo dequeue(int num) throws Exception {
      ...
      while(true) {
          ...
          if(best_crawl_url == null) {
            ...
            System.out.println("Thread " + Thread.currentThread().getId() + "get url is null");
            break;
          }
          // get url
          ...
          UrlText ut = database.getUrl(encoded_url);
          if( (ut != null && ut.isVisited()) || best_crawl_url.getDepth() >= DEPTH) {
          ...
          System.out.println("Url: " + url + "; Robot Url: " + robotUrl + " is null: " + (rt == null));
          ...
          //5. the server have robots.txt
              ...
              if(! robotParser.allowUrl(rt, url)) {
                  System.out.println("Allow Url: " + url);
                  continue;
              }
      ...
      System.out.println("Best Url: " + best_crawl_url.getUrl() + "; Best delay: " + best_delay);
  ...
  private HeadNode inspectUrl(String absolute_url, String encoded_url) {
      ...
      while(head_node != null && head_node.getLocation() != null && i > 0) {
          ...
          UrlText ut = database.getUrl(encoded_url);
          if(! UrlNormalizer.schemaIsHttp(absolute_url) || (ut != null) && ut.isVisited())
              return null;
  ...
  private HeadNode checkHeader(String url, String encoded_url) {
      // Get HtmlDocument 
      System.out.println("Thread " + Thread.currentThread().getId() + " Check URL: " + url);
  ...
  private Document downloadUrl(HeadNode head_node) throws Exception {
      ...
      if(head_node.getState() == 1) {
          System.out.println("Thread " + Thread.currentThread().getId() + " " + absolute_url + ": Not Modified");
          ...
          if(database.getContent(encoded_content) == null) {
              ...
              System.out.println("Thread " + Thread.currentThread().getId() + " " + absolute_url + ": Parsing");
      ...
      // Case 2. response is ok
        ...
        if(! head_node.typeValid() || (int)(head_node.getLength() / Math.pow(10, 6)) > max_size) {
          ...
          if(database.getContent(encoded_content) == null) {
              database.storeContent(encoded_content, encoded_url);
              System.out.println("Thread " + Thread.currentThread().getId() + " " + absolute_url + ": Downloading");
              // 4. increment 1, we retrieve the document
              crawler.incrementCount();
              // 5. store in the database, maybe different url store same content
              database.storeHtmlDocument(encoded_url, absolute_url, content, new Date());
              return doc;
          }
          else {
            System.out.println("Thread " + Thread.currentThread().getId() + " visited the content");
            return null;
          }
        }
      }
