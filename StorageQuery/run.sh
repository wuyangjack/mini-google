...
case "$1" in
	...
	DumpBody)
		...
		THREADS=5
		KEY=1
		java -cp $CLASSPATH $DUMPER $SOURCE_BODY $DATABASE $CIS455_NODES $CIS455_INDEX $TABLE_BODY $THREADS $KEY
		;;
	DumpModTitle)
		cd $ROOT
		THREADS=5
	...
	DumpModBody)
		...
		THREADS=5
		KEY=0
		java -cp $CLASSPATH $APP $SOURCE_MODBODY $DATABASE $CIS455_NODES $CIS455_INDEX $TABLE_MODBODY $THREADS $KEY
		;;
	*)
		echo "Unknown mode"
		;;
