...
public class BerkeleyDB {
	...
	// singleton class
	private static BerkeleyDB database;
	...
	public static BerkeleyDB getInstance() {
		if(database == null)
			database = new BerkeleyDB();
		return database;
	}
	
	public static boolean setEnvPath1(String env_path) {
		BerkeleyDB.env_path = env_path;
		return true;
	}
	...
	public void openDB() {
		 try {
			...
			//open TF-IDF DB 
				tf_idf = this.dbEnv.openDatabase(null, "tf_idf", config);
...
//						
			...
			OperationStatus status = this.tf_idf.put(null, stringToEntry(word), stringToEntry(value.toString()));
	...
	public ArrayList<WordDocTFEntry> getTFIDF(String word){
		...
		try{
			...
			cursor = this.tf_idf.openCursor(null, null);
			...
			while (retVal == OperationStatus.SUCCESS) {
				...
				System.out.println("I am DB entry raw: " + dataString); 
				
				
				JSONObject value = new JSONObject(dataString);
